...
namespace xhttper
{
	...
	public:
		...
		http_builder &append_entry(const std::string &key, std::string &&value)
		{
			headers_.emplace_back(key, std::move(value));
			return *this;
		}
		http_builder &append_entry(const std::string &key, const std::string &value)
		{
			headers_.emplace_back(key, std::move(value));
			return *this;
		}
		...
		static std::string get_content_type(const std::string &ext)
		{
			...
			static const std::map<const char *, const char *, cmper> extensions =
			{
				{ ".323", "text/h323" },
				{ ".3gp", "video/3gpp" },
			...
			auto itr = extensions.find(ext.c_str());
